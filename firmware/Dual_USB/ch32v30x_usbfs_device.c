                ...
                switch ( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP2:
                        ...
                        /* 鼠标协议解析: 逐字节解析收到的数据 */
                        {
                            ...
                            uint16_t i;
                            /* 指向上一包的缓冲区 (Tx_LoadNum 已经 +1, 所以上一包是 Tx_LoadNum-1) */
                            uint8_t *pbuf = (uint8_t *)&UART2_Tx_Buf[ ((Uart.Tx_LoadNum == 0) ? (DEF_UARTx_TX_BUF_NUM_MAX-1) : (Uart.Tx_LoadNum-1)) * DEF_USB_FS_PACK_LEN ];
                            extern void Mouse_Packet_Parse(uint8_t data);
                            for(i = 0; i < rxlen; i++)
                            {
                                Mouse_Packet_Parse(pbuf[i]);
                            }
                        }
                        ...
                        /* end-point 4 data out interrupt (禁用回环, 不处理) */
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP4:
                          USBFSD->UEP4_RX_CTRL ^= USBFS_UEP_R_TOG;
                          /* 丢弃收到的数据, 不再回环 */
                          USBFSD->UEP4_RX_CTRL = (USBFSD->UEP4_RX_CTRL & ~USBFS_UEP_R_RES_MASK) | USBFS_UEP_R_RES_ACK;
                        break;
