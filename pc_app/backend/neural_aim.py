...
class TinyNN:
    ...
    def __init__(self, lr: float = 0.01, y_scale: float = 0.3, max_step_px: float = 10.0):
        ...
        # 性能评估
        self._error_history: list = []          # 最近 60 帧的误差
        self._best_avg_error: float = 1e9       # 最佳平均误差
        self._best_weights: Optional[dict] = None  # 最佳权重备份
        self._saved_path: str = ''               # 已保存的路径
    ...
    def train_step(self, error_now: float, error_before: float):
        ...
        if self._train_count >= 60:  # 有足够数据才开始评估
            ...
            if avg_err < self._best_avg_error:
                ...
                logger.info(f'NN new best avg_error={avg_err:.1f}px at step {self._train_count}')
                # 平均误差 < 8px 且增益 > 0.5 时自动保存
                if avg_err < 8.0 and self._gain > 0.5 and not self._saved_path:
                    ...
                    logger.info(f'✅ NN auto-saved to {path} (avg_err={avg_err:.1f}px)')
        ...
        if self._train_count % 30 == 0:
            ...
            self._last_status = (f'🧠 train={self._train_count} gain={self._gain:.2f} '
                                 f'avg_err={self._best_avg_error:.1f}px '
